...
export default function Auth() {
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden px-5 py-12">
      ...
      <Link
        to="/"
        className="absolute left-5 top-6 inline-flex items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-foreground sm:left-8"
      >
      ...
      <section className="relative z-10 w-full max-w-md border border-border bg-card/80 p-6 shadow-2xl shadow-primary/5 backdrop-blur-xl sm:p-9">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {message && (
            <p
              role="status"
              className="border-l-2 border-success bg-success/10 px-3 py-2 text-sm text-success"
            >
              {message}
            </p>
          )}
