...
import { ArrowRight, BellRing, CircleDollarSign, Eye, Plane, TicketCheck } from "lucide-react";
...
export default function Index() {
  usePageMeta(
    ...
    "設定航線與目標價，機票降價就通知你。Track flights and get an email when fares drop.",
  );
    ...
    <main className="min-h-screen overflow-hidden bg-background">
      ...
      <section className="relative mx-auto grid min-h-[670px] max-w-7xl items-center gap-12 px-5 pb-24 pt-16 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:px-12 lg:pb-28 lg:pt-20">
        ...
        <div className="relative z-10 animate-rise">
          ...
          <p className="mt-7 text-xl font-medium leading-relaxed text-foreground sm:text-2xl">
            設定航線與目標價，機票降價就通知你
          </p>
          ...
          <Button asChild size="lg" className="mt-9 h-12 px-6 shadow-lg shadow-primary/15">
            <Link to="/auth">
              Start watching fares <ArrowRight />
            </Link>
          </Button>
        </div>

        <div
          className="relative z-10 mx-auto w-full max-w-lg animate-rise-delayed"
          aria-label="Flight price tracking preview"
        >
          <div className="flight-panel">
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <span className="size-2 rounded-full bg-success" /> LIVE WATCH
              </div>
            ...
            <div className="p-5 sm:p-7">
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4">
                <div>
                  <p className="text-4xl font-semibold">TPE</p>
                  <p className="mt-1 text-xs text-muted-foreground">Taipei</p>
                </div>
                ...
                <div className="text-right">
                  <p className="text-4xl font-semibold">NRT</p>
              ...
              <div className="my-7 h-28 border-y border-border py-4">
                <svg
                  ...
                  role="img"
                  aria-label="Fare price dropping below target"
                >
                  <path
                    d="M4 19 C45 13,60 34,96 30 S148 53,181 43 S226 66,260 57 S317 77,396 68"
                    fill="none"
                    stroke="var(--primary)"
                    strokeWidth="3"
                  />
              ...
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <p className="mt-1 text-3xl font-semibold">NT$ 8,420</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">YOUR TARGET</p>
                  <p className="mt-1 font-mono text-sm text-success">NT$ 9,000 ✓</p>
                </div>
              </div>
            </div>
          </div>
          <div className="absolute -bottom-5 -right-2 flex items-center gap-3 border border-success/25 bg-card px-4 py-3 shadow-xl sm:-right-6">
            <TicketCheck className="size-5 text-success" />
            <div>
              <p className="text-xs font-medium">Price target reached</p>
              <p className="text-[11px] text-muted-foreground">Email alert sent</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 px-5 py-16 sm:px-8 md:flex-row md:items-center lg:px-12">
        <div>
          <p className="text-2xl font-semibold">下一次降價，不要再錯過。</p>
          <p className="mt-2 text-muted-foreground">Your next trip starts with the right fare.</p>
        </div>
